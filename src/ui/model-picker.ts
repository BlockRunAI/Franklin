...
export function listPickerModelsFlat(): ModelEntry[] {
 ...
 */
export function formatModelPickerListText(currentModel?: string): string {
  const lines: string[] = [];
  let idx = 1;
  for (const cat of PICKER_MODELS) {
    ...
    for (const m of cat.models) {
      ...
      lines.push(
        `  ${String(idx).padStart(2)}. ${m.label.padEnd(26)} ${m.shortcut.padEnd(14)} ${m.price}${cur}`
      );
      idx++;
    }
    lines.push('');
