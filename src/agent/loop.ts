...
export async function interactiveSession(
  ...
  while (true) {
    ...
    turnFailedModels = new Set<string>(); // Fresh slate for this turn
    ...
    while (loopCount < maxTurns) {
      ...
      try {
        ...
        if (classified.isTransient && recoveryAttempts < 3) {
          ...
          if (config.debug) {
            console.error(
              `[franklin] ${classified.label} error — retrying in ${(backoffMs / 1000).toFixed(1)}s (attempt ${recoveryAttempts}): ${errMsg.slice(0, 100)}`
            );
          }
          onEvent({
            kind: 'text_delta',
