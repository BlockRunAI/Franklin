...
listSessionSandboxes, terminateAllSessionSandboxes, } from '@blockrun/franklin/vscode-session';
...
export function activate(context) {
    ...
    try {
        ...
        if (!process.env.FRANKLIN_CLI_PATH) {
            log.appendLine(`[Franklin] WARNING: could not locate franklin/dist/index.js next to extension. ` +
...
class FranklinChatProvider {
     ...
     * memory for Revert.
     */
    maybeSendEditDiff(result) {
        if (!this.webview || !result.diff)
            return;
    ...
    async openTradingDashboard() {
        ...
        if (hasLocal && nodeBin) {
            cmd = nodeBin;
            args = [localDist, 'panel', '--port', String(PORT)];
        }
        else if (franklinBin) {
            cmd = franklinBin;
            args = ['panel', '--port', String(PORT)];
        }
        else {
        ...
        let spawnError = '';
        proc.stderr?.on('data', (d) => { spawnError += String(d); });
        proc.on('error', (err) => {
    ...
    cancelTask(runId) {
        ...
        try {
            ...
            try {
                ...
                void this.webview?.postMessage({ type: 'taskCancelResult', runId, ok: false, reason: err.message });
            }
        }
        catch (e) {
    ...
    async sendWalletQr() {
        try {
            ...
            const chain = w.chain === 'solana' ? 'solana' : 'base';
     ...
     */
    async importSession(source, externalId) {
    ...
    async applySettings(settings) {
        ...
        if (img && img !== '__unset__')
            config['default-image-model'] = img;
        else
            delete config['default-image-model'];
        if (vid && vid !== '__unset__')
            config['default-video-model'] = vid;
        else
            delete config['default-video-model'];
    ...
    async runAgentSession() {
        ...
        const onAskUser = (question, options) => new Promise((resolve) => {
...
function getWebviewHtml(webview, extensionUri) {
    ...
    const mascotUri = webview.asWebviewUri(vscode.Uri.joinPath(extensionUri, 'media', 'franklin-mascot-transparent.png'));
    const portraitUri = webview.asWebviewUri(vscode.Uri.joinPath(extensionUri, 'media', 'franklin-portrait.jpg'));
