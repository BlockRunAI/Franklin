...
| 9 | Bash guard (destructive command warning) | `src/agent/bash-guard.ts` | "Agent executes `rm -rf` / `git push --force` / DROP TABLE without realizing" | Safety | **Permanent** — safety layer, not capability layer | Not a candidate |
